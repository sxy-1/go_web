...
//评论操作
func (mgr *manager) AddComment(comment *model.Answer) {
	mgr.db.Create(comment)
}

func (mgr *manager) GetComment(username string) []model.Answer {
	var comment = make([]model.Comment, 10)
	mgr.db.Table("comments").Where("writer=?", username).Find(&answer)
	return comment
}
